...
export default function Snippet({ item, allComments }: SnippetsProps) {
    ...
    async function handleComment(event: FormEvent) {
        ...
        try {
            ...
            if (item.userMail !== session?.user?.email) {
                await addDoc(collection(db, 'notifications'), {
                    notification: `${session?.user?.name} comentou no seu snippet ${item.title}`,
                    snippetIdNotification: item.id,
                    createdNotified: new Date(),
                    userMailNotified: item.userMail,
                })
            }
